...
class CctvSpider(scrapy.Spider):
    ...
    def __init__(self, path):
        self.all_shows = [
            ...
            ['32 中国梦 祖国颂2017', 'http://tv.cctv.com/2017/09/26/VIDAGMO9c7wSn9PNgP88nv8F170926.shtml', '', '精彩看点', 6, True],
            ['33 强军战旗红——建军90周年“心连心”赴南昌慰问演出', 'http://tv.cctv.com/2017/07/31/VIDAyJUXRTNc4saRFK5YLKvk170731.shtml', '', '精彩看点', 6, True],
    ...
    def get_url(self, web_type, codes, page):
        ...
        if web_type == 0:
            ...
            year = codes[2]
            month = codes[3]
            if year != self.year_now:
                date_query = '&Y=' + year + '&M=' + month
            else:
                date_query = '&Y=' + year
            url = self.api_data['url'] + 'id=' + codes[0] + '&n=' + str(self.api_data['items']) \
                + '&of=fdate&p=' + page + '&type=1&serviceId=tvcctv' + date_query
        elif web_type == 15:
    ...
    def parse(self, response):
        ...
        if web_type == 0 or web_type == 14 or web_type == 15:
            ...
            if page == 1:
                ...
                if total_pages > 1:
                    page_limit = 2
                    if web_type == 0 or web_type == 15:
                        page_limit = total_pages + 1
                    elif web_type == 14:
                        page_limit = total_pages + 2
                    for p_idx in range(2, page_limit):
                        if p_idx == total_pages + 1:
                            page_query = 1
                            if codes[3] is None or int(codes[3]) == 1:
                                codes[3] = str(12)
                                if int(codes[2]) > 2012:
                                    codes[2] = str(int(codes[2]) - 1)
                                else:
                                    break
                            else:
                                codes[3] = str(int(codes[3]) - 1)
                        else:
                            page_query = p_idx
                        url = self.get_url(web_type, codes, page_query)
                        yield scrapy.Request(
                            url=url,
                            callback=self.parse,
                            errback=self.parse_error,
